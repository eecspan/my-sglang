...
class ModelRunner:
    ...
    def _refresh_kv_cache_scales(self) -> None:
        ...
        for _, module in self.model.named_modules():
            ...
            k_scale_float = getattr(module, "k_scale_float", None)
            v_scale_float = getattr(module, "v_scale_float", None)
            if (
                k_scale is None
                or v_scale is None
                or k_scale_float is None
                or v_scale_float is None
            ):
                continue
            if not isinstance(k_scale, torch.Tensor) or not isinstance(v_scale, torch.Tensor):
                continue
